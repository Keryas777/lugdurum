(() => {
  ...
  const normalizeProduct = (raw, index) => {
    const code = String(raw.parfum_code || "").trim().toUpperCase();
  ...
  const render = () => {
    const products = state.catalogue
      ...
      .sort((a, b) =>
        ...
        a.parfum_code.localeCompare(b.parfum_code) ||
        (b.format_cl - a.format_cl)
      );

    els.activeCount.textContent = String(products.filter((product) => product.actif).length);
    els.offersCount.textContent = String(state.offres.filter((offer) => offer.actif).length);
      ...
      return `
        <article class="catalogueCard">
          <div class="catalogueCardHeader">
            ...
            <strong class="catalogueAmount">${offer ? escapeHtml(formatCurrency(offer.prix_ttc)) : "—"}</strong>
          </div>
          <div class="catalogueMeta">
            <span class="catalogueBadge ${statusClass}">${escapeHtml(statusLabel)}</span>
  ...
  const loadData = async () => {
    ...
    try {
      if (!api()?.getCatalogue || !api()?.getOffresVente) throw new Error("API catalogue indisponible.");
